        ...
        <template #buttons>
            <div class="column row-md items-end q-gutter-sm">
                <button-group class="col-auto">
                    ...
                    <q-btn flat icon="sym_o_more_vert" color="primary" class="cursor-pointer button-border" @click.stop>
                        <q-menu v-if="file" auto-close>
                            <q-list>
                                <q-item clickable v-ripple :disable="isInvalid" @click="emit('copy-link')">
                                    <q-item-section avatar><q-icon name="sym_o_content_copy" /></q-item-section>
                                    <q-item-section>Copy public link</q-item-section>
                                </q-item>
                                <q-item clickable v-ripple :disable="!file?.hash" @click="emit('copy-hash')">
                                    <q-item-section avatar><q-icon name="sym_o_encrypted" /></q-item-section>
                                    <q-item-section>Copy MD5</q-item-section>
                                </q-item>
                                <q-item clickable v-ripple @click="emit('copy-uuid')">
                                    <q-item-section avatar><q-icon name="sym_o_fingerprint" /></q-item-section>
                                    <q-item-section>Copy UUID</q-item-section>
                                </q-item>
                                <q-item clickable v-ripple class="text-negative">
                                    <q-item-section avatar><q-icon name="sym_o_delete" /></q-item-section>
                                    <q-item-section>
                                        <DeleteFileDialogOpener v-if="file" :file="file">
                                            Delete File
            ...
            <div class="q-gutter-md q-mt-xs">
                <div class="row items-start q-gutter-y-sm">
                    ...
                    <div class="col-12 col-md-1">
                        ...
                        <q-icon :name="getIcon(file?.state ?? FileState.OK)" :color="getColorFileState(file?.state ?? FileState.OK)" size="sm">
                ...
                <div class="row items-start q-gutter-xs">
                    <q-chip v-for="cat in file?.categories" :key="cat.uuid" :label="cat.name"
                            :style="{ backgroundColor: hashUUIDtoColor(cat.uuid) }" text-color="white" size="sm" />
                </div>
            </div>
        </template>
    </title-section>
...
import { getColorFileState, getIcon, getTooltip, hashUUIDtoColor } from 'src/services/generic';
...
const isDownloadDisabled = computed(() =>
    [FileState.LOST, FileState.UPLOADING].includes(props.file?.state ?? FileState.LOST)
);
const isInvalid = computed(() =>
    props.file?.state === FileState.LOST || props.file?.state === FileState.ERROR
);
</script>

<style scoped>
.text-placeholder { font-size: 12px; color: #666; }
